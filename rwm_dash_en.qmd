...
rwm3 <- read_parquet("data/data_clean23.parquet") %>% 
  select(date, participants, state) %>%
...
mainPanel(
  ...
  h5(HTML(
    ...
    <ul style='text-align: left; display: inline-block;'>
     <li><strong>Overview:</strong> A static, downloadable wall poster summarizing the main findings.</li>
      <li><strong>Map:</strong> An interactive county-level mobilization map showing demonstrations classified by the Domestic Intelligence Service as right-wing extremist.</li>
      <li><strong>Timeline:</strong> An interactive aggregation of demonstrations at the federal state level, illustrating their development over time.</li>
      <li><strong>Protest Sizes:</strong> An interactive graphic depicting indiviudal protest size by federal state across time.</li>
    </ul>
    <p>Please explore each section to dive deeper into the data.</p>

    <br>
    <br>
    <br> 
           <a href='https://data.gesis.org/sharing/#!Detail/10.7802/2256' target='_blank' style='font-size: 16px;'> Data is available at: Kanol, E.; Knoesel, J. (2021): Right-Wing Extremist Mobilization in Germany. Version 1.0.0. WZB Berlin Social Science Center. Dataset.</a>"
...
mainPanel(
  ...
  h5(HTML("This visualization displays the number of right-wing extremist demonstrations and participants by county from 2005 through 2022. <br>
           <a href='https://data.gesis.org/sharing/#!Detail/10.7802/2256' target='_blank'style='font-size: 12px;'> Source: Kanol, E.; Knoesel, J. (2021): Right-Wing Extremist Mobilization in Germany. Version 1.0.0. WZB Berlin Social Science Center. Dataset.</a>"))
)
```
...
mainPanel(
  ...
  h5(HTML("This chart presents the number of right-wing extremist demonstrations and participants by state from 2005 through 2022. While the data can be disaggregated daily for time-series analysis, we have opted for annual aggregation to enhance clarity. <br> 
           <a href='https://data.gesis.org/sharing/#!Detail/10.7802/2256' target='_blank', style='font-size: 12px;'> Source: Kanol, E.; Knoesel, J. (2021): Right-Wing Extremist Mobilization in Germany. Version 1.0.0. WZB Berlin Social Science Center. Dataset.</a>")),
...
mainPanel(
  ...
  h3("Right-wing extremist protest sizes by federal state over time"),
  h5(HTML("This visualization shows the scope and frequency of right-wing extremist demonstrations by federal state. <br>
           <a href='https://data.gesis.org/sharing/#!Detail/10.7802/2256' target= '_blank' style='font-size: 12px;'> Source: Kanol, E.; Knoesel, J. (2021): Right-Wing Extremist Mobilization in Germany. Version 1.0.0. WZB Berlin Social Science Center. Dataset.</a>"))
)
```
...
```{r}
sliderInput("bubbleTimeRange", "Select Time Range:",
            min = min(rwm3$year, na.rm = TRUE),
            max = max(rwm3$year, na.rm = TRUE),
            value = c(min(rwm3$year, na.rm = TRUE), max(rwm3$year, na.rm = TRUE)),
            step = 1,
            sep = "",
            ticks = FALSE)

checkboxGroupInput("bubbleStates", "Select Federal States:",
                   choices = c(
                   ...
                   selected = c("BW", "BY", "BE", "BB", "HB", "HH", "HE", "NI",
...
output$bubblePlot <- renderPlotly({
    ...
    mutate(
      ...
      yearmonth = as.numeric(format(date, "%Y%m"))
    )

  # Filter the data based on user inputs
  state_filtered <- rwm3 %>%
    filter(!is.na(participants) & !is.na(yearmonth)) %>%
    filter(year >= input$bubbleTimeRange[1] & year <= input$bubbleTimeRange[2]) %>%
    filter(state %in% input$bubbleStates) %>%
    mutate(
      xpos = as.integer(factor(yearmonth, levels = sort(unique(yearmonth))))
    )

  # Calculate unique breaks and labels for the x-axis
  unique_mappings <- state_filtered %>%
    ungroup() %>%
    select(xpos, yearmonth) %>%
    distinct() %>%
    mutate(
      year = floor(yearmonth / 100)
    ) %>%
    group_by(year) %>%
    summarise(xpos = min(xpos)) %>%
    ungroup()

  # Determine which years to display based on the selected time range
  displayed_years <- seq(input$bubbleTimeRange[1], input$bubbleTimeRange[2], by = 1)
  unique_mappings <- unique_mappings %>% filter(year %in% displayed_years)

  unique_breaks <- unique_mappings$xpos
  unique_labels <- unique_mappings$year

  # Create tooltip text
  state_filtered <- state_filtered %>%
    mutate(tooltip_text = paste0(
      "Date: ", format(date, "%Y-%m-%d"), "<br>",
      "Federal State: ", state, "<br>",
      "Participants: ", participants
    ))

  # Create the ggplot
  g <- ggplot(state_filtered, aes(
      x = xpos, y = state, size = participants, text = tooltip_text
    )) +
    geom_point(alpha = 0.4, shape = 21, aes(color = state)) +  # Dynamic coloring by state
    theme_minimal() +
    scale_y_discrete(
      labels = c(
    ...
    scale_x_continuous(
      breaks = unique_breaks, labels = unique_labels
    ) +
    scale_size_continuous(
      range = c(3, 25),  # Adjust the range for better visibility
  ...
  ggplotly(g, tooltip = "text") %>%
    layout(
      ...
      yaxis = list(title = "Federal State"),
